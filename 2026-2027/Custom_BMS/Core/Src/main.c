...
int main(void)
{
  ...
  MX_TIM6_Init();
  /* USER CODE BEGIN 2 */
  /* Force PA7 (MOSI) to Open-Drain manually */
...
//	  	  HAL_SPI_TransmitReceive(&hspi1, TX_Data, sizeof(TX_Data));
//	  //}
//	  SPI_frame(TX_Data, packet_init_table[SINGLE_R], device_addr_table[BRIDGE], 0x2100, (uint8_t[]){0x00}, 1); // Read a register
//
...
//	  HAL_Delay(200);
	  ...
	  //HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_SET);
	  HAL_Delay(500);
	  uint8_t test_read_data[1] = {0xFF};
	  int bruh = reg_addr_table[DIR0_ADDR];
	  BQ79600_ReadReg(packet_init_table[SINGLE_R], device_addr_table[BRIDGE], reg_addr_table[DIR0_ADDR], test_read_data, 1);
...
uint16_t crc16(uint8_t *data, uint16_t len) {
    uint16_t crc = 0xFFFF; // Standard TI starting value

    for (uint16_t i = 0; i < len; i++) {
        crc ^= (uint16_t)(data[i] << 8); // XOR byte into the MSB of the CRC

        for (uint8_t bit = 0; bit < 8; bit++) {
            if ((crc & 0x8000) != 0) {
                // If the MSB is 1, shift left and XOR with the IBM polynomial
                crc = (crc << 1) ^ 0x8005;
            } else {
...
void BQ79600_WriteReg(uint8_t packet_type, int8_t device_addr, uint16_t reg_addr, uint8_t *data, uint8_t size) {

    SPI_frame(TX_Data, packet_type, device_addr, reg_addr, data, size);

    uint8_t frame_len = (device_addr >= 0) ? (6 + size) : (5 + size);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET);

    HAL_SPI_Transmit(&hspi1, TX_Data, frame_len, HAL_MAX_DELAY);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET);
}

void BQ79600_ReadReg(uint8_t packet_type, int8_t device_addr, uint16_t reg_addr, uint8_t *rx_data, uint8_t read_size) {
    // data is number of bytes requested - 1
	uint8_t req_data[1] = { read_size - 1 };
	SPI_frame(TX_Data, packet_type, device_addr, reg_addr, req_data, 1);
    ...
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET);
    // delay
    delay_us(5 * NUM_BOARDS);

    // # bits to be read
    uint16_t expected_rx_len = read_size + 6;
    if (packet_type == packet_init_table[STACK_R]) {expected_rx_len = expected_rx_len * NUM_BOARDS;}
    else if (packet_type == packet_init_table[BROADCAST_W]) {expected_rx_len = expected_rx_len * NUM_BOARDS;}

    // if SPI_RDY then loop and read 128 bytes at a time while transmitting FF
    uint8_t dummy_tx[128];
	memset(dummy_tx, 0xFF, sizeof(dummy_tx));

	uint8_t raw_rx_buffer[1024];
	uint8_t *movingPointer = raw_rx_buffer;

    int iter = expected_rx_len / 128;
    int len_last = expected_rx_len % 128;
    while (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_3) == 0) {
		delay_us(1);
	}
    ...
    while (iter >= 0){
    	if (iter == 0){
    		HAL_SPI_TransmitReceive(&hspi1, dummy_tx, movingPointer, len_last, HAL_MAX_DELAY);
    	}
    	else{
    		HAL_SPI_TransmitReceive(&hspi1, dummy_tx, movingPointer, 128, HAL_MAX_DELAY);
    	}
    	movingPointer += 128;
    	iter--;
    }
    ...
    bytes_received = expected_rx_len;
    memcpy(rx_data, &raw_rx_buffer[4], read_size);
}
...
void Comm_Init(){
  ...
  HAL_Delay(4);
  payload[0] = 0x20;
  BQ79600_WriteReg(packet_init_table[SINGLE_W], device_addr_table[BRIDGE], reg_addr_table[CONTROL1], payload, 1);
  // then wait ((~1.6 + 10ms) * n stacked devices)
  HAL_Delay(12);

  // set back to alternate function mode
  GPIO_InitTypeDef GPIO_InitStruct = {0};
  GPIO_InitStruct.Pin = GPIO_PIN_7; // MOSI
  GPIO_InitStruct.Mode = GPIO_MODE_AF_PP; // Set back to Alternate Function
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
  GPIO_InitStruct.Alternate = GPIO_AF5_SPI1; // Check your datasheet, usually AF5 for SPI1
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

  // device address self-assignment: sync delay-locked loop
  payload[0] = 0x00;
  BQ79600_WriteReg(packet_init_table[STACK_W], -1, 0x0343, payload, 1); // OPT_ECC_DATAIN1
  BQ79600_WriteReg(packet_init_table[STACK_W], -1, 0x0344, payload, 1); // OPT_ECC_DATAIN2
  BQ79600_WriteReg(packet_init_table[STACK_W], -1, 0x0345, payload, 1); // OPT_ECC_DATAIN3
  BQ79600_WriteReg(packet_init_table[STACK_W], -1, 0x0346, payload, 1); // OPT_ECC_DATAIN4
  BQ79600_WriteReg(packet_init_table[STACK_W], -1, 0x0347, payload, 1); // OPT_ECC_DATAIN5
  BQ79600_WriteReg(packet_init_table[STACK_W], -1, 0x0348, payload, 1); // OPT_ECC_DATAIN6
  BQ79600_WriteReg(packet_init_table[STACK_W], -1, 0x0349, payload, 1); // OPT_ECC_DATAIN7
  BQ79600_WriteReg(packet_init_table[STACK_W], -1, 0x034A, payload, 1); // OPT_ECC_DATAIN8

  payload[0] = 0x01;
  BQ79600_WriteReg(packet_init_table[BROADCAST_W], -1, reg_addr_table[CONTROL1], payload, 1);  // enable auto addressing

  payload[0] = 0x00;
  BQ79600_WriteReg(packet_init_table[BROADCAST_W], -1, reg_addr_table[DIR0_ADDR], payload, 1); // set bridge device to address 0

  payload[0] = 0x01;
  BQ79600_WriteReg(packet_init_table[BROADCAST_W], -1, reg_addr_table[DIR0_ADDR], payload, 1); // set first stack device to address 1
}
